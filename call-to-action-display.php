...
function ctaw_display_action($before_widget, $after_widget, $before_title, $after_title, $hide_title = false) {
    ...
    foreach ($ctaws as $ctaw) {
        ...
        if($front_page && is_front_page())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if($single_page && is_single())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if($home_page && is_home())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if($archive_page && is_archive())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if($author_page && is_author())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if($error_page && is_404())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if($search_page && is_search())
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        if ($single_post && count($action_categories) == 0) {
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        } else if (is_single() && $single_post && array_intersect($current_categories, $action_categories)) {
            array_push($possible_actions, array($ctaw->ID, $ctaw->post_title, $ctaw->post_content));
        }
    }
